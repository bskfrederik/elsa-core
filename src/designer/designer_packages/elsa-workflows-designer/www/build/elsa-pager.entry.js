...
const PreviousButton = ({ currentPage, pageCount, onNavigate }) => currentPage > 0 ?
  h("a", { href: "#", onClick: e => clickHandler(e, () => onNavigate(currentPage - 1)), class: "elsa-pager-button previous" }, "Previous") : undefined;
const NextButton = ({ currentPage, pageCount, onNavigate }) => currentPage < pageCount ?
  h("a", { href: "#", onClick: e => clickHandler(e, () => onNavigate(currentPage + 1)), class: "elsa-pager-button next" }, "Next") : undefined;
...
const ElsaPager = class {
  ...
  render() {
    ...
    return (h("div", { class: "tw-bg-white tw-px-4 tw-py-3 tw-flex tw-items-center tw-justify-between tw-border-t tw-border-gray-200 sm:tw-px-6" }, h("div", { class: "tw-flex-1 tw-flex tw-justify-between sm:hidden" }, h(PreviousButton, { currentPage: page, pageCount: pageCount, onNavigate: this.onNavigate }), h(NextButton, { currentPage: page, pageCount: pageCount, onNavigate: this.onNavigate })), h("div", { class: "hidden sm:tw-flex-1 sm:tw-flex sm:tw-items-center sm:tw-justify-between" }, h("div", null, h("p", { class: "tw-text-sm tw-leading-5 tw-text-gray-700 tw-space-x-0.5" }, h("span", null, "From"), h("span", { class: "tw-font-medium" }, from), h("span", null, "to"), h("span", { class: "tw-font-medium" }, to), h("span", null, "of"), h("span", { class: "tw-font-medium" }, totalCount), h("span", null, "results"))), h("div", null, h("nav", { class: "tw-relative tw-z-0 tw-inline-flex tw-shadow-sm" }, h(ChevronLeft, { currentPage: page, pageCount: pageCount, onNavigate: this.onNavigate }), h(PagerButtons, { currentPage: page, pageCount: pageCount, onNavigate: this.onNavigate }), h(ChevronRight, { currentPage: page, pageCount: pageCount, onNavigate: this.onNavigate }))))));
  }
};
