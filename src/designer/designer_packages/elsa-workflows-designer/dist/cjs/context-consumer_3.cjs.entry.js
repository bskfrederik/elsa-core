...
const SyntaxSelectorIcon = () => index.h("svg", { class: "tw-h-5 tw-w-5 tw-text-gray-400", width: "24", height: "24", viewBox: "0 0 24 24", "stroke-width": "2", stroke: "currentColor", fill: "none", "stroke-linecap": "round", "stroke-linejoin": "round" },
  index.h("path", { stroke: "none", d: "M0 0h24v24H0z" }),
  index.h("circle", { cx: "12", cy: "12", r: "9" }),
  index.h("line", { x1: "8", y1: "12", x2: "8", y2: "12.01" }),
  index.h("line", { x1: "12", y1: "12", x2: "12", y2: "12.01" }),
  index.h("line", { x1: "16", y1: "12", x2: "16", y2: "12.01" }));
...
const InputControlSwitch = class {
  constructor(hostRef) {
    ...
    this.renderContextMenuButton = () => this.shouldRenderMonaco() ? index.h("span", null, this.syntax) : index.h(SyntaxSelectorIcon, null);
    this.renderEditor = () => {
      ...
      return (index.h("div", null, index.h("div", { class: expressionEditorClass }, index.h("elsa-monaco-editor", { value: value, language: monacoLanguage, "editor-height": this.codeEditorHeight, "single-line": this.codeEditorSingleLineMode, onValueChanged: e => this.onExpressionChangedDebounced(e.detail), ref: el => this.monacoEditor = el })), index.h("div", { class: defaultEditorClass }, index.h("slot", null)), index.h(hint.Hint, { text: this.hint })));
    };
    ...
    this.defaultSyntax = utils.SyntaxNames.Literal;
    ...
    this.onExpressionChangedDebounced = lodash.lodash.debounce(this.onExpressionChanged, 10);
  }
  ...
  async componentDidLoad() {
    const elsaClient = await utils.Container.get(utils.ElsaClientProvider).getElsaClient();
  ...
  render() {
    if (this.hideLabel && !this.shouldRenderMonaco()) {
      return index.h("div", { class: "tw-p-4" }, index.h("div", { class: "tw-flex" }, index.h("div", { class: "tw-flex-1" }, this.renderEditor()), this.renderContextMenuWidget()));
    }
    return index.h("div", { class: "tw-p-4" }, index.h("div", { class: "tw-mb-1" }, index.h("div", { class: "tw-flex" }, index.h("div", { class: "tw-flex-1" }, this.renderLabel()), this.renderContextMenuWidget())), this.renderEditor());
  }
  renderLabel() {
    const fieldId = this.fieldName;
    const fieldLabel = this.label || fieldId;
    return index.h("label", { htmlFor: fieldId, class: "tw-block tw-text-sm tw-font-medium tw-text-gray-700" }, fieldLabel);
  }
  renderContextMenuWidget() {
    ...
    return index.h("div", { class: "tw-relative", ref: el => this.contextMenuWidget = el }, index.h("button", { type: "button", class: `tw-border-0 focus:tw-outline-none tw-text-sm ${advancedButtonClass}`, onClick: e => this.onSettingsClick(e) }, !this.isReadOnly ? this.renderContextMenuButton() : ""), index.h("div", null, index.h("div", { ref: el => this.contextMenu = el, "data-transition-enter": "tw-transition tw-ease-out tw-duration-100", "data-transition-enter-start": "tw-transform tw-opacity-0 tw-scale-95", "data-transition-enter-end": "tw-transform tw-opacity-100 tw-scale-100", "data-transition-leave": "tw-transition tw-ease-in tw-duration-75", "data-transition-leave-start": "tw-transform tw-opacity-100 tw-scale-100", "data-transition-leave-end": "tw-transform tw-opacity-0 tw-scale-95", class: "hidden tw-origin-top-right tw-absolute tw-right-1 tw-mt-1 tw-w-56 tw-rounded-md tw-shadow-lg tw-bg-white tw-ring-1 tw-ring-black tw-ring-opacity-5 tw-divide-y tw-divide-gray-100 focus:tw-outline-none tw-z-10", role: "menu", "aria-orientation": "vertical", "aria-labelledby": "options-menu" }, index.h("div", { class: "tw-py-1", role: "none" }, index.h("a", { onClick: e => this.selectSyntax(e, null), href: "#", class: `tw-block tw-px-4 tw-py-2 tw-text-sm hover:tw-bg-gray-100 hover:tw-text-gray-900 ${!selectedSyntax ? 'tw-text-blue-700' : 'tw-text-gray-700'}`, role: "menuitem" }, "Default")), index.h("div", { class: "tw-py-1", role: "none" }, this.supportedSyntaxes.map(syntax => (index.h("a", { onClick: e => this.selectSyntax(e, syntax), href: "#", class: `tw-block tw-px-4 tw-py-2 tw-text-sm hover:tw-bg-gray-100 hover:tw-text-gray-900 ${selectedSyntax == syntax ? 'tw-text-blue-700' : 'tw-text-gray-700'}`, role: "menuitem" }, syntax)))))));
  }
  toggleContextMenu() {
...
state.InputControlSwitchContextState.injectProps(InputControlSwitch, ['workflowDefinitionId', 'activityType', 'propertyName']);
...
async function initializeMonacoWorker(libPath) {
  return await mutex.runExclusive(async () => {
    ...
    if (utils.isNullOrWhitespace(libPath)) {
    ...
    require$1.config({ paths: { 'vs': `${baseUrl}/vs` } });
...
`], { type: 'text/javascript' }));
    return new Promise(resolve => {
      require$1(["vs/editor/editor.main"], () => {
        isInitialized = true;
...
const ElsaMonaco = class {
  ...
  async getMonaco() {
    if (!!this.monaco)
      return this.monaco;
    const monacoLibPath = utils.state$4.monacoLibPath;
  ...
  render() {
    ...
    return (index.h(index.Host, { class: "monaco-editor-host tw-border focus:tw-ring-blue-500 focus:tw-border-blue-500 tw-block tw-w-full tw-min-w-0 tw-rounded-md sm:tw-text-sm tw-border-gray-300 tw-p-4", style: { 'min-height': this.editorHeight } }, index.h("div", { ref: el => this.container = el, class: `monaco-editor-container ${padding}` })));
  }
...
exports.elsa_input_control_switch = InputControlSwitch;
exports.elsa_monaco_editor = ElsaMonaco;

//# sourceMappingURL=context-consumer_3.cjs.entry.js.map