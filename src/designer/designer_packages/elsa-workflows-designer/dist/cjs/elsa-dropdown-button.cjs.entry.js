...
const DropdownButton = class {
  constructor(hostRef) {
    ...
    this.menuOpened = index.createEvent(this, "menuOpened", 7);
  ...
  render() {
    ...
    return (index.h(index.Host, { class: "tw-block", ref: el => this.element = el }, index.h("span", { class: "tw-relative tw-z-0 tw-inline-flex tw-shadow-sm tw-rounded-md" }, index.h("button", { type: "button", disabled: disabled, class: `tw-relative tw-inline-flex tw-items-center tw-px-4 tw-py-2 tw-rounded-l-md tw-border tw-text-sm tw-font-medium focus:tw-z-10 focus:tw-outline-none ${buttonClass}`, onClick: handler }, this.renderIcon(), this.text), index.h("div", { class: "-tw-ml-px tw-block" }, index.h("button", { type: "button", disabled: disabled, class: `tw-relative tw-inline-flex tw-items-center tw-px-2 tw-py-2 tw-rounded-r-md tw-border tw-text-sm tw-font-medium focus:tw-z-10 focus:tw-outline-none ${arrowClass}`, onClick: () => this.toggleMenu(), "aria-expanded": "true", "aria-haspopup": "true" }, index.h("svg", { class: "tw-h-5 tw-w-5", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 20 20", fill: "currentColor", "aria-hidden": "true" }, index.h("path", { "fill-rule": "evenodd", d: "M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z", "clip-rule": "evenodd" }))), this.renderMenu()))));
  }
  renderMenu() {
    if (this.items.length == 0)
      return;
    const originClass = this.getOriginClass();
    return index.h("div", { ref: el => this.contextMenu = el, "data-transition-enter": "tw-transition tw-ease-out tw-duration-100", "data-transition-enter-start": "tw-transform tw-opacity-0 tw-scale-95", "data-transition-leave": "tw-transition tw-ease-in tw-duration-75", "data-transition-leave-start": "tw-transform tw-opacity-100 tw-scale-100", "data-transition-leave-end": "tw-transform tw-opacity-0 tw-scale-95", class: `hidden ${originClass} tw-z-10 tw-absolute tw-mt-2 tw-w-56 tw-rounded-md tw-shadow-lg tw-bg-white tw-ring-1 tw-ring-black tw-ring-opacity-5` }, index.h("div", { class: "tw-py-1", role: "menu", "aria-orientation": "vertical" }, this.renderItems()));
  }
  renderItems() {
    const groups = lodash.lodash.groupBy(this.items, x => { var _a; return (_a = x.group) !== null && _a !== void 0 ? _a : 0; });
    return index.h("div", { class: "tw-divide-y tw-divide-gray-100 focus:tw-outline-none", role: "menu", "aria-orientation": "vertical", "aria-labelledby": "option-menu" }, lodash.lodash.map(groups, menuItemGroup => {
      return index.h("div", { class: "tw-py-1", role: "none" }, menuItemGroup.map(menuItem => {
        ...
        return (index.h("div", { class: "tw-py-1", role: "none" }, index.h("a", { href: "#", onClick: e => this.onItemClick(e, menuItem), class: `tw-block tw-px-4 tw-py-2 tw-text-sm ${selectedCssClass}`, role: "menuitem" }, menuItem.text)));
      }));
    }));
  }
  closeContextMenu() {
...
exports.elsa_dropdown_button = DropdownButton;

//# sourceMappingURL=elsa-dropdown-button.cjs.entry.js.map